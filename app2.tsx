...
/* ---- scopes ---------------------------------------------------------------
   ...
   shows 28 at once, so it needs a map. Kept SEPARATE and additive: nothing here
...
function pScopeFor(key, cfg, span, wantsDSA) {
  let s = PSCOPES[key];
  if (s) return s;
  const need = Math.max(Math.round(span * FS), FS * 4);
  s = { eng: makeEngine(3), cfg, span, buf: new Float64Array(need), i: 0, t: 0, acc: 0, cols: [], dsa: wantsDSA };
  ...
  for (let k = 0; k < need; k++) { s.buf[k] = s.eng(s.t, cfg, 0); s.t += 1 / FS; }
  if (wantsDSA) {
    const per = Math.round(PCOL_S * FS);
    for (let c = 0; c < PNCOL; c++) {
      for (let j = 0; j < per; j++) {
        s.buf[s.i] = s.eng(s.t, cfg, 0);
...
function PrimerScope({ spec, height, small }) {
  ...
  useEffect(() => {
    ...
    const step = (now) => {
      ...
      const s = pScopeFor(sp.key, sp.cfg, sp.span, sp.dsa);
      if (dt) pAdvance(s, dt);
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            {c.ctrl.opts.map((o, i) => (
              <button key={String(o[1])} onClick={() => setOpt(i)} aria-pressed={i === optIdx} style={{
                flex: "1 1 28%", minWidth: 84, minHeight: 44, borderRadius: 10, cursor: "pointer",
