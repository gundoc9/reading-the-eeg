...
/* The v1 palette and its three font constants used to live here. Nothing has
   referenced them since the phone-first rebuild — measured at 0 uses — and a
   second dead palette beside a live theme table is a trap for the next edit. */
...
const KEY = [
  ...
  { p: 0.64, n: "Analgesia given", d: "Alpha returns without changing the hypnotic. That supports the nociceptive reading; it does not settle it.",        a: { occAlpha: 0, front18: 0.5, emg: 0.01, slow: 32, delta: 14, theta: 1, froAlpha: 17, beta: 1 } },
...
const COMBOS = {
  ...
  "propofol+ketamine": {
    ...
    sourced: true,
    note: "Ketamine does not fade the propofol alpha, it moves it. The peak shifts up out of the alpha band into beta, and the slow-delta falls with it. Watch the stripe climb the axis rather than dim. Described by Kim 2020, reporting Hayashi's finding; that primary paper is not held here.",
    comps: [
      { f: 0.8, amp: 24, topo: "flat", mode: "sustained", band: "slow", shape: "slow" },
      { f: 2.2, amp: 9, topo: "flat", mode: "sustained", band: "delta" },
      { f: 5.2, amp: 6, topo: "front", mode: "sustained", band: "theta" },
      { f: 15.0, amp: 11, topo: "front", mode: "sustained", band: "beta" },
...
const REFS = {
  ...
  purdon13: {
    ...
    full: "Purdon, Pierce, Mukamel, Prerau, Walsh, Wong, Salazar-Gomez, Harrell, Sampson, Cimenser, Ching, Kopell, Tavares-Stoeckel, Habeeb, Merhar, Brown. Electroencephalogram signatures of loss and recovery of consciousness from propofol. Proc Natl Acad Sci U S A 2013;110(12):E1142-51. Source of the anterior alpha shift.",
    doi: "10.1073/pnas.1221180110",
    note: "Citation now complete; the primary text is still not held, so everything taught from it here comes second-hand through Purdon 2015 and Guay 2025. Read it before teaching from it.",
  },
  markus: {
  ...
  kim20: {
    s: "Kim 2020", ok: true,
    full: "Kim, Fricchione, Brown, Akeju. Role of electroencephalogram oscillations and the spectrogram in monitoring anaesthesia. BJA Education 2020;20(5):166-172. A review, not primary research. Carries a corrigendum at BJA Education 2020;20(7):249 correcting three figure cross-references in the text.",
    doi: "10.1016/j.bjae.2020.01.004",
    note: "Source of the propofol-plus-ketamine morphology used here, which it reports from Hayashi 2007. That primary paper is not held.",
  },
  sewell19: {
    s: "Sewell 2019", ok: true,
    full: "Sewell, Abbas, Kane. Introduction to interpretation of the EEG in intensive care. BJA Education 2019;19(3):74-82. A review, not primary research, written for critical care rather than theatre.",
    doi: "10.1016/j.bjae.2018.11.002",
    note: "Source for burst suppression having causes other than the anaesthetic. It also uses the critical-care glossary, in which suppression is under 10 microvolts; this programme teaches Guay's 5. Two thresholds are in circulation and the difference is not settled here.",
  },
  paeds: {
    s: "Paediatric set", ok: false,
    full: "Long, Lim, Balanza, Allen, Purdon, Bong. Sevoflurane requirements during electroencephalogram (EEG)-guided vs standard anesthesia care in children: a randomized controlled trial. J Clin Anesth 2022;81:110913, doi 10.1016/j.jclinane.2022.110913. And Kratzer, Davidson. Processed EEGs in children; are they useful? Pediatric Anesthesia 2025;35(5):336-337, doi 10.1111/pan.15089 — an editorial, not a study. Yoon, Markus, Derylo and Bong & Yuan are held in full and cited separately.",
    doi: "",
    note: "Both citations are now complete. Neither primary text is held here, so nothing in this programme should rest on either without reading them first.",
  },
  convention: {
...
const MODULES = [
  ...
  },
  {
    ...
    points: [
      ...
      { t: "Add ketamine to propofol and the alpha does not fade, it moves: the peak climbs out of the alpha band into beta, and the slow-delta falls with it.", r: "kim20", cfg: { combo: "propofol+ketamine" }, ctrl: "combo", look: "Watch the stripe climb the axis rather than dim. Put it beside the propofol alpha in Four agents, four signatures." },
...
/* The bank only ever grows by APPENDING. prog.drill.box is keyed by index, so
   inserting an item would silently shift every stored Leitner box for everyone
   who has already used the app. A gate enforces it. v18 added six so that no
   module sits below three — module 01 had one, and the module check asks
   min(3, pool.length) questions, so finishing it gave a one-question check. */
...
const DRILLS = [
  ...
  { mod: "01", cfg: { state: "propofol", ch: 0, age: 82 }, q: "Standard propofol maintenance dose. What does this record tell you about the dose?",
    opts: ["It is too high", "It is too low", "Nothing — dose is the input, this is the output", "It is correct for the weight"],
    a: "Nothing — dose is the input, this is the output",
    why: "Two patients on the same dose of the same drug can sit in different states, so the record reports where this brain has arrived, not whether the number on the pump was right. Reading it the other way round is how a normal dose gets defended in front of an abnormal record." },
  { mod: "01", cfg: { state: "ketamine", ch: 0 }, q: "The blood pressure and heart rate are up. Does this record support \"too light\"?",
    opts: ["Yes", "No — this is an anaesthetised record", "Cannot say from the EEG", "Only if the alpha returns"],
    a: "No — this is an anaesthetised record",
    why: "Gamma alternating with slow-delta is the ketamine signature at depth (Akeju 2016). Whatever is driving the haemodynamics, the record is not showing arousal — and that divergence is the argument for reading the organ rather than the variables downstream of it." },
  { mod: "02", cfg: { state: "propofol", ch: 0, scale: 500 }, q: "At 500 µV full scale this trace looks small. Is the amplitude low?",
    opts: ["Yes", "No — read the µV number", "Only for this age", "Cannot tell without the DSA"],
    a: "No — read the µV number",
    why: "Around 100 µV peak to peak, which is ordinary maintenance. The vertical scale is a setting; the readout is the fact. Since amplitude changes five- to twentyfold between states, a fixed gain misleads you in one direction or the other most of the time." },
  { mod: "07", cfg: { state: "dexmedetomidine", ch: 0 }, q: "Frontal, around 13 Hz. Is \"frontal alpha\" the right name for it?",
    opts: ["Yes, it is frontal and rhythmic", "No — it is spindle activity, and continuity is the difference", "Yes, if the patient is anaesthetised", "No — it is beta"],
    a: "No — it is spindle activity, and continuity is the difference",
    why: "Two things separate them and neither is topography. It sits at 13 Hz, above this programme's 8 to 12 Hz alpha band — under one per cent of the power here is inside it. And it arrives in discrete runs with gaps, where propofol's alpha is continuous. Solid stripe or broken stripe is the discriminator (Akeju 2014)." },
  { mod: "11", cfg: { state: "propofol", ch: 0, bsr: 1 }, q: "No bursts at all, and the trace is small but not a straight line. What is this?",
    opts: ["Burst suppression", "Attenuation", "Electrocerebral inactivity if it measures 2 µV or less", "A disconnected electrode"],
    a: "Electrocerebral inactivity if it measures 2 µV or less",
    why: "Burst suppression needs bursts. With none, and amplitude at or below 2 µV, the term is electrocerebral inactivity — a different statement, made from the µV readout rather than from how flat it looks. Gain alone can make either look like the other (Guay 2025)." },
  { mod: "12", cfg: { state: "sevoflurane", ch: 0 }, q: "Your monitor's index reads 42. The spectrogram beside it shows slow-delta, theta and a frontal alpha. Which is the primary observation?",
    opts: ["The index — it is validated", "The spectrum — the index is computed from it", "Whichever matches the vaporiser", "Neither on its own"],
    a: "The spectrum — the index is computed from it",
    why: "An index is an abstract of the spectrum you are already looking at. Where the two disagree, the record is the observation and the number is the summary of it — which is why this programme shows SEF95, which is defined and computable, and no proprietary index at all." },
  { mod: "11", cfg: { state: "propofol", ch: 0, bsr: 0.5 }, q: "Burst suppression on a standard dose. What does this record tell you about the cause?",
    opts: ["The dose is too high", "The patient is hypothermic", "Nothing — the record cannot name the cause", "There has been a seizure"],
    a: "Nothing — the record cannot name the cause",
    why: "The anaesthetic is the commonest cause and the first thing to check. But a discontinuous record looks the same whether it comes from drug, hypothermia, cerebral ischaemia or the period after a seizure (Sewell 2019). The EEG gives you the state, not the reason — which is why the next move is to look at the patient and not only at the vaporiser." },
];
...
const LOOKUP = [
  ...
  },
  {
    ...
    foot: "Suggested targets for propofol or sevoflurane in children, from a narrative review rather than a trial. No association with age was found for the measured values. Unreliable under three months.",
  },
];
...
/* ---- the author's other teaching apps, listed on the About page ----------- */
const SAME_AUTHOR = {
  title: "More from the same author",
  items: [
    ["Reading the clot", "TEG and ROTEM, one term per card, with the normal trace drawn beside the abnormal one.", "gundoc9.github.io/reading-the-clot"],
    ["Cannulation geometry", "Insertion angle, catheter length, and how far the tip reaches into the vein.", "gundoc9.github.io/cannulation-geometry"],
    ["How the AI you use works", "The words for what happens once your prompt is inside the model.", "gundoc9.github.io/how-the-ai-works"],
  ],
};
...
const EMPTY_PROG = { done: {}, last: null, seenIntro: false, theme: "dark", primer: { seen: false, at: 0 }, drill: { seen: 0, right: 0, byMod: {}, box: {}, idx: 0 } };
...
/* ---- themes --------------------------------------------------------------
   Two palettes for one instrument. The dark theme is the monitor in a darkened
   theatre; the paper theme is a neurophysiology chart — ink on stock, with the
   pen colours doing the work the amber does in the dark. The density spectral
   array keeps a dark inset in BOTH, because jet needs a dark floor to read and
   a spectrogram is a screen object, not a paper one.

   P is MUTATED IN PLACE, never reassigned. Every inline style object is built
   during render and the scope reads P inside its animation loop, so a theme
   change reaches all of them on the next frame WITHOUT remounting anything.
   That matters: remounting Scope would destroy the canvas 2d context and the
   drawn spectrogram along with it.

   Every colour literal in this file lives in the table below. A gate enforces
   it, because a stray hex is a value that only one theme can be right about. */

const THEMES = {
  dark: {
    label: "Instrument",
    note: "Dark. Built for theatre light and for reading a trace at night.",
    bg: "#0E1116",
    surface: "#171B22",
    surface2: "#212831",
    line: "#2C3540",
  ...
  paper: {
    label: "Chart paper",
    note: "Light. For a bright room, a shared screen or a projector.",
    bg: "#E3E7E1",
    surface: "#FBFBF8",
    surface2: "#EDF0EA",
    line: "#C3C9BF",
...
const P = { name: "dark", ...THEMES.dark };

function rgbOf(hex) {
  const h = String(hex).replace("#", "");
  return [parseInt(h.slice(0, 2), 16), parseInt(h.slice(2, 4), 16), parseInt(h.slice(4, 6), 16)];
}
// A wash of a token at low alpha, so tinted surfaces follow the theme instead
// of being frozen at whatever the dark palette happened to be.
function tint(hex, a) {
  const c = rgbOf(hex);
...
function applyTheme(name) {
  ...
  for (const k in THEMES[t]) P[k] = THEMES[t][k];
  P.name = t;
  // the page behind #root, and the iOS status bar, are outside React
  try {
    if (typeof document !== "undefined" && document.body) {
      document.body.style.background = P.bg;
      // the html element shows during overscroll bounce on iOS; left at the
      // boot colour it flashed a dark band above and below a chart-paper page
      if (document.documentElement) document.documentElement.style.background = P.bg;
      const m = document.querySelector('meta[name="theme-color"]');
      if (m) m.setAttribute("content", P.bg);
    }
  } catch { /* no document in the test harness */ }
  return t;
}

// Read the stored theme synchronously at load where we can, so a paper user
// does not get a dark flash on every launch. window.storage is async by design;
// localStorage is not, and the standalone build is backed by it.
function bootTheme() {
  try {
    const raw = localStorage.getItem(KEY_STORE);
    if (raw) applyTheme(JSON.parse(raw).theme || "dark");
  } catch { /* first run, private mode, or no localStorage */ }
}
bootTheme();

// Bump both by hand when the content changes. Without them a colleague who hits
// a problem cannot say which build, and there is no way to issue a correction.
const VERSION = "v28";
const BUILT = "17 September 2026";
...
// The spectrogram fills one column every 0.25 s and holds 150 of them, so an
// unprimed scope spends 37.5 s as a small block in the corner of an empty panel
// while the landing text says everything above is live and running. Generate
// that history up front, through the SAME code path the live loop uses, so the
// instrument opens full. Costs one synchronous pass at first mount and nothing
// afterwards — SCOPE is a singleton and this runs once.
function primeScope(cfg) {
  if (SCOPE.cols.length) return;
  const c = resolveCfg(cfg);
  const ch = c.ch ?? 0;
  const step = Math.round(0.25 * FS);
  const buf = SCOPE.buf, win = SCOPE.win;
  // fill the ring buffer first, so no column is computed over silence
  for (let i = 0; i < buf.length; i++) {
    SCOPE.t += 1 / FS;
    buf[SCOPE.wr] = SCOPE.eng(SCOPE.t, c, ch);
    SCOPE.wr = (SCOPE.wr + 1) % buf.length;
  }
  for (let n = 0; n < 150 * step; n++) {
    SCOPE.t += 1 / FS;
    buf[SCOPE.wr] = SCOPE.eng(SCOPE.t, c, ch);
    SCOPE.wr = (SCOPE.wr + 1) % buf.length;
    if ((n + 1) % step === 0) {
      win.set(buf.subarray(SCOPE.wr));
      win.set(buf.subarray(0, SCOPE.wr), buf.length - SCOPE.wr);
      SCOPE.cols.push(spectrum(win.subarray(win.length - WIN_N), NFFT, 3));
    }
  }
}
...
function Scope({ cfg, trace = 104, spec = 104, onPause }) {
  ...
  useEffect(() => {
    ...
    const step = (now) => {
      ...
      tctx.strokeStyle = tint(P.accent, P.gridA);
      tctx.lineWidth = 1;
      ...
      for (let x = 0; x < NCOL; x++) {
        ...
        for (let y = 0; y < NBIN; y++) {
          const o = (y * NCOL + x) * 4;
          if (!col) { img.data[o] = se[0]; img.data[o + 1] = se[1]; img.data[o + 2] = se[2]; img.data[o + 3] = 255; continue; }
  ...
  return (
    <div style={{ background: P.surface, borderRadius: 14, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 12, padding: "2px 13px 4px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        {/* On the isolated-element screen the background is deliberately near
            silent, so a spectral edge and a burst suppression ratio measured
            over it describe the demonstration, not an EEG. The BSR read 33 to
            50 per cent in red beside a K-complex; the element's own duration
            and peak are the readout that screen is for. */}
        {cfg.state === "quiet" ? null : <span style={chip}>SEF95 {ro.sef.toFixed(1)}</span>}
        {cfg.state === "quiet" ? null : <span style={{ ...chip, color: ro.bsr > 0.05 ? P.warn : P.dim }}>BSR {(ro.bsr * 100).toFixed(0)}%</span>}
        {cfg.transient ? (
...
/* App installs its theme setter here on render. The alternative was threading
   two props through eight screens to reach one 40px button; this renderer has
   no context API, and there is exactly one App, so a module-level pointer is
   the honest version of the same thing. ThemeButton holds no state of its own —
   the re-render that follows a change comes from App, and the button simply
   reads P.name again on the way through. */
let setThemeFromAnywhere = () => {};

function ThemeButton() {
  const keys = Object.keys(THEMES);
  const next = keys[(keys.indexOf(P.name) + 1) % keys.length];
  return (
    <button onClick={() => setThemeFromAnywhere(next)} title={`Switch to ${THEMES[next].label}`}
      aria-label={`Switch to the ${THEMES[next].label} theme`} style={{
        background: P.surface2, border: "none", borderRadius: 10, width: 44, height: 44,
        cursor: "pointer", flexShrink: 0, padding: 0,
        display: "inline-flex", alignItems: "center", justifyContent: "center",
      }}>
      {/* the swatch is the thing itself: the ground and the ink of the theme you
          are in, split down the middle. Not a sun and a moon — this programme
          has no icon set and is not about to acquire one for a single control. */}
      <svg width="20" height="20" viewBox="0 0 20 20" aria-hidden="true">
        <circle cx="10" cy="10" r="8.4" fill={P.surface} stroke={P.dim} strokeWidth="1.4" />
        <path d="M10 1.6 A8.4 8.4 0 0 1 10 18.4 Z" fill={P.ink} />
      </svg>
    </button>
  );
}

/* An outbound link is not an outbound FETCH. The page still needs no network to
   run; tapping one opens a new tab on a deliberate action. The build gate now
   distinguishes the two, and the privacy wording says so. */
function Link({ href, children, style }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer"
      style={{ color: P.accent, textDecoration: "underline",
               textUnderlineOffset: 2, ...(style || {}) }}>{children}</a>
  );
}
/* Every outbound destination is a literal here, so the build can audit the list
   rather than guess at a runtime-built string. A site not on it simply is not
   tappable — it fails safe rather than opening somewhere unaudited. */
const OUTBOUND = {
  "youtube.com/@eegforanesthesia3954": "https://youtube.com/@eegforanesthesia3954",
  "pedseeg.com": "https://pedseeg.com",
  "gundoc9.github.io/reading-the-clot": "https://gundoc9.github.io/reading-the-clot/",
  "gundoc9.github.io/cannulation-geometry": "https://gundoc9.github.io/cannulation-geometry/",
  "gundoc9.github.io/how-the-ai-works": "https://gundoc9.github.io/how-the-ai-works/",
};
const DOI_RE = /\b(10\.\d{4,9}\/[^\s,;)\]]+)/;
// render a line of prose with any doi inside it turned into a link
function withDoi(text, key) {
  // every doi in the string becomes a link, not only the first: the paediatric
  // set in Sources carries two in its text and both were plain
  const out = [];
  let rest = text, n = 0;
  for (;;) {
    const m = DOI_RE.exec(rest);
    if (!m) { out.push(rest); break; }
    out.push(rest.slice(0, m.index));
    out.push(React.createElement(Link, { href: "https://doi.org/" + m[1], key: "d" + key + "-" + n++ }, m[1]));
...
function Pill({ children, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      borderRadius: 999, minHeight: 44, padding: "0 15px",
...
function CaseScrubber({ cfg, setCfg }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.5, color: P.ink, margin: "10px 0 12px" }}>{ph.d}</p>
      <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, WebkitOverflowScrolling: "touch" }}>
        {CASE_KEYS.map((k) => (
          <button key={k.n} onClick={() => setCfg({ ...cfg, casePos: k.p })} style={{
            ...
            borderRadius: 999, minHeight: 44, padding: "0 14px", border: "none",
  ...
  return (
    <div>
      ...
      <div style={{ background: P.surface2, borderRadius: 12, padding: "12px 14px" }}>
        ...
        <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.5, color: P.ink, margin: 0 }}>{c.note}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ background: P.surface2, borderRadius: 12, padding: "12px 14px" }}>
        ...
        <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.5, color: P.ink, margin: 0 }}>{b.note}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ background: P.surface2, borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
        ...
        <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.5, color: P.ink, margin: 0 }}>{spec.note}</p>
      </div>
...
function Landing({ onStart, onSkip, onPrimer }) {
  const mins = MODULES.reduce((a, m) => a + m.mins, 0);
  // These four are the first prose anyone reads, and one of them is the promise
  // that nothing is downloaded. Full strength, like every other paragraph.
  const fact = { fontFamily: UI, fontSize: 14.5, lineHeight: 1.5, color: P.ink, margin: "0 0 9px", paddingLeft: 15, position: "relative" };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "18px 0 8px" }}>
        <p style={{ flex: 1, minWidth: 0, fontFamily: UI, fontSize: 11.5, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: P.accent, margin: 0 }}>
      ...
      <div style={{ background: P.surface, borderRadius: 14, padding: "16px 17px", marginBottom: 18 }}>
        ...
        <p style={fact}><span style={dot} />Nothing is downloaded and nothing is sent anywhere. Your progress stays on this device. Links open in a new tab, and only if you tap one.</p>
        <p style={{ ...fact, margin: 0 }}><span style={dot} />Mapped to the ESAIC 2026 learning outcomes, but it cannot certify you against them.</p>
      </div>

      <Btn kind="primary" size="lg" full onClick={onPrimer}>Start with the basics</Btn>
      <div style={{ marginTop: 10 }}>
        <Btn kind="ghost" full onClick={onStart}>I know the vocabulary — start at module 01</Btn>
      </div>
      <div style={{ marginTop: 10 }}>
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, gap: 10 }}>
        ...
        <button onClick={() => onOpen(5, 0)} style={{
          ...
          fontFamily: UI, fontSize: 13, fontWeight: 600, color: P.accent, minHeight: 44,
        }}>Skip the foundations ›</button>
      </div>
      <p style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.5, color: P.ink, margin: "0 0 12px" }}>
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        ...
        <button onClick={() => setNotes(!notes)} aria-label="Teaching notes" style={{
          background: notes ? P.accent : P.surface2, border: "none", borderRadius: 10, minWidth: 44, height: 44,
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        <h1 style={{ flex: 1, minWidth: 0, fontFamily: UI, fontSize: 21, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>
          {only ? "Module check" : "Drill"}
        </h1>
        <span style={{ fontFamily: MN, fontSize: 12, color: P.dim, flexShrink: 0 }}>
        ...
        {d.opts.map((o) => {
          ...
          return (
            <button key={o} onClick={() => answer(o)} style={{
              ...
              background: isA ? tint(P.ok, 0.14) : isW ? tint(P.warn, 0.14) : P.surface,
...
function BenchScreen({ cfg, setCfg, narrow }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        <h1 style={{ flex: 1, fontFamily: UI, fontSize: 21, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>Bench</h1>
        <ThemeButton />
      </div>
      ...
      <p style={{ fontFamily: UI, fontSize: 14, lineHeight: 1.55, color: P.ink, marginTop: 18 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
        <h1 style={{ flex: 1, fontFamily: UI, fontSize: 21, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>Reference</h1>
        <ThemeButton />
      </div>
      <p style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.5, color: P.ink, margin: "0 0 16px" }}>
              ...
              <div style={{ padding: "0 16px 14px" }}>
                {card.rows.map((row) => (
                  <div key={row[0]} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "7px 0", borderTop: `1px solid ${P.line}` }}>
                    <span style={{ fontFamily: UI, fontSize: 14.5, color: P.dim, flex: "0 0 42%", lineHeight: 1.35 }}>{row[0]}</span>
                    <span style={{ fontFamily: UI, fontSize: 14.5, fontWeight: 600, color: P.ink, flex: 1, lineHeight: 1.35 }}>{row[1]}</span>
                  </div>
                ))}
                {card.foot ? (
                  <p style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.5, color: P.ink, margin: "11px 0 0", paddingTop: 10, borderTop: `1px solid ${P.line}` }}>{card.foot}</p>
                ) : null}
      ...
      <div style={{ background: P.surface, borderRadius: 14, padding: "15px 16px", marginTop: 14, borderLeft: `3px solid ${P.accent}` }}>
        ...
        {REAL_RECORDS.links.map((l) => (
          <div key={l[0]} style={{ marginBottom: 10 }}>
            ...
            <span style={{ display: "block", fontFamily: UI, fontSize: 13.5, lineHeight: 1.45, color: P.ink, margin: "2px 0" }}>{l[1]}</span>
            {OUTBOUND[l[2]]
              ? <Link href={OUTBOUND[l[2]]} style={{ display: "block", fontFamily: MN, fontSize: 12 }}>{l[2]}</Link>
              : <span style={{ display: "block", fontFamily: MN, fontSize: 12, color: P.dim }}>{l[2]}</span>}
          </div>
        ))}
      </div>
    </div>
...
function MoreScreen({ prog, persist, theme, onTheme, onShowIntro }) {
  ...
  // "out of scope" used the rule colour, which measured 1.4:1 against the card
  // where a meaningful indicator needs 3:1 — so the one state this panel most
  // needs to be honest about was the one you could not see. dim clears it.
  // Three filled circles told apart by hue alone are unreadable to a colour-blind
  // eye. Shape now carries the meaning and colour reinforces it: disc, square, ring.
  const dotStyle = (s) => (s === 2 ? { background: P.ok, borderRadius: 4 }
    : s === 1 ? { background: P.accent, borderRadius: 1 }
    : { background: "transparent", border: `2px solid ${P.dim}`, borderRadius: 4 });
  const word = (s) => (s === 2 ? "covered" : s === 1 ? "partly covered" : "out of scope");
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        <h1 style={{ flex: 1, fontFamily: UI, fontSize: 21, fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>More</h1>
        <ThemeButton />
      </div>
      ...
      {panel === "framework" && (
        <>
          <p style={{ fontFamily: UI, fontSize: 18, fontWeight: 600, margin: "0 0 10px", lineHeight: 1.35 }}>
            {n2} covered · {n1} partly · {n0} out of scope
          </p>
          <div style={{ display: "flex", gap: 15, flexWrap: "wrap", margin: "0 0 16px" }}>
            {[[2, "covered"], [1, "partly"], [0, "out of scope"]].map(([s, w]) => (
              <span key={w} style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: UI, fontSize: 12.5, color: P.dim }}>
                <span style={{ width: 9, height: 9, flexShrink: 0, ...dotStyle(s) }} />{w}
              </span>
            ))}
          </div>
          <p style={{ fontFamily: UI, fontSize: 14, lineHeight: 1.55, color: P.ink, margin: "0 0 20px" }}>
          ...
          {FRAMEWORK.map(([level, dim, items]) => (
            <div key={level + dim} style={{ marginBottom: 18 }}>
              <h3 style={{ fontFamily: UI, fontSize: 11.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: P.dim, margin: "0 0 8px" }}>{level} · {dim}</h3>
              {items.map(([code, text, status, where]) => (
                <div key={code} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: P.surface, borderRadius: 10, padding: "10px 12px", marginBottom: 6 }}>
                  <span aria-label={word(status)} title={word(status)} style={{ width: 9, height: 9, flexShrink: 0, marginTop: 6, ...dotStyle(status) }} />
        ...
        <>
          ...
          {Object.entries(REFS).map(([k, r]) => (
            <div key={k} style={{ background: P.surface, borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
              ...
              <p style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.5, color: P.ink, margin: 0 }}>{withDoi(r.full, k)}</p>
              {r.doi ? <p style={{ fontFamily: MN, fontSize: 11.5, color: P.dim, margin: "4px 0 0" }}>doi <Link href={"https://doi.org/" + r.doi}>{r.doi}</Link></p> : null}
              {r.note ? <p style={{ fontFamily: UI, fontSize: 13, lineHeight: 1.5, color: P.warn, margin: "5px 0 0" }}>{r.note}</p> : null}
            </div>
          ))}
          <p style={{ fontFamily: UI, fontSize: 13, lineHeight: 1.5, color: P.ink, margin: "12px 0 0" }}>
      ...
      {panel === "about" && (
        <div>
          ...
          <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.55, color: P.ink, margin: "0 0 14px" }}>
            ...
            outcomes only and sets no assessment standard. Nothing is downloaded and no data leaves this device. The doi and site links open in a new tab, and only when you tap them.
          </p>
          <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.55, color: P.ink, margin: "0 0 16px" }}>
          ...
          <div style={{ padding: "14px 0 16px", marginBottom: 16, borderTop: `1px solid ${P.line}`, borderBottom: `1px solid ${P.line}` }}>
            <p style={{ fontFamily: UI, fontSize: 11.5, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: P.dim, margin: "0 0 9px" }}>Appearance</p>
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 9 }}>
              {Object.entries(THEMES).map(([k, t]) => (
                <Pill key={k} active={theme === k} onClick={() => onTheme(k)}>{t.label}</Pill>
              ))}
            </div>
            <p style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.5, color: P.dim, margin: 0 }}>
              {THEMES[theme].note} Same modules, same readings — only the colours change, and the choice stays on this device.
            </p>
          </div>

          <div style={{ padding: "0 0 16px", marginBottom: 16, borderBottom: `1px solid ${P.line}` }}>
            ...
            <p style={{ fontFamily: MN, fontSize: 11.5, color: P.dim, margin: "9px 0 0" }}>{VERSION} · built {BUILT}</p>
          </div>

          <div style={{ padding: "0 0 16px", marginBottom: 16, borderBottom: `1px solid ${P.line}` }}>
            <p style={{ fontFamily: UI, fontSize: 11.5, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: P.dim, margin: "0 0 10px" }}>{SAME_AUTHOR.title}</p>
            {SAME_AUTHOR.items.map((l) => (
              <div key={l[0]} style={{ marginBottom: 12 }}>
                <span style={{ display: "block", fontFamily: UI, fontSize: 15.5, fontWeight: 600, color: P.ink }}>{l[0]}</span>
                <span style={{ display: "block", fontFamily: UI, fontSize: 13.5, lineHeight: 1.45, color: P.ink, margin: "2px 0" }}>{l[1]}</span>
                {OUTBOUND[l[2]]
                  ? <Link href={OUTBOUND[l[2]]} style={{ display: "block", fontFamily: MN, fontSize: 12 }}>{l[2]}</Link>
                  : <span style={{ display: "block", fontFamily: MN, fontSize: 12, color: P.dim }}>{l[2]}</span>}
          ...
          <Btn kind="ghost" full onClick={onShowIntro}>Show the opening screen again</Btn>
        </div>
      )}

      {panel === "reading" && (
        <>
          <p style={{ fontFamily: UI, fontSize: 14.5, lineHeight: 1.55, color: P.ink, margin: "0 0 16px" }}>{FURTHER.note}</p>
          {FURTHER.items.map((it) => (
            <div key={it.doi} style={{ background: P.surface, borderRadius: 12, padding: "13px 15px", marginBottom: 8 }}>
              ...
              <p style={{ fontFamily: MN, fontSize: 11.5, color: P.dim, margin: 0 }}>doi <Link href={"https://doi.org/" + it.doi}>{it.doi}</Link></p>
            </div>
          ))}
        </>
      )}
      ...
      {panel === "reset" && (
        <div>
          <p style={{ fontFamily: UI, fontSize: 15, lineHeight: 1.55, color: P.ink, margin: "0 0 16px" }}>
    ...
    <div style={{ minHeight: "100vh", background: P.bg, color: P.ink, fontFamily: UI, padding: "22px 20px 40px" }}>
      <div style={{ maxWidth: 940, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <span style={{ flex: 1, minWidth: 0, fontFamily: UI, fontSize: 14, fontWeight: 600, color: P.accent }}>{mod.n} · {mod.title}</span>
        ...
        <p style={{ fontSize: 15, lineHeight: 1.55, color: P.ink, marginTop: 22, paddingTop: 16, borderTop: `1px solid ${P.line}` }}>{mod.prompt}</p>
...
/* Generated from the tested primer build. Do not hand-edit: regenerate. */
const PRIMER_CARDS = [
 {
  "id": "raw-trace",
  "topic": "The two pictures",
  "title": "The raw trace",
  "tldr": "The wiggly line. Voltage at the forehead, drawn against time.",
  "lede": "A seismograph does not know what the earthquake was. It records how the ground moved, and everything else is read off that.",
  "body": [
   {
    "p": "The raw trace is the same kind of instrument. It plots the voltage difference between two electrodes against time, in microvolts. Millionths of a volt. An ECG deals in millivolts, a thousand times larger, which is most of why the EEG is so much fussier about electrode contact."
   },
   {
    "h": "What it is good at"
   },
   {
    "p": "Shape, timing, and anything sudden. Every named thing in this dictionary is named on the raw trace, because a name is a claim about a shape and this is the only picture that shows shape."
   },
   {
    "h": "What it hides"
   },
   {
    "p": "Anything slow. You cannot watch a line for forty minutes and notice that a rhythm has been gradually fading, because by the time it has faded the earlier picture is gone. That is the job of the second picture."
   }
  ],
  "src": [
   "General signal description. The microvolt and millivolt figures are standard and are not taken from any paper held here."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 {
  "id": "amplitude",
  "topic": "Describing what you see",
  "title": "Amplitude",
  "tldr": "How big a wave is, measured in microvolts from its bottom to its top.",
  "lede": "Wave height at sea is measured trough to crest, not from an imagined flat sea level, because the flat level is a fiction and the trough is a thing you can point at.",
  "body": [
   {
    "p": "EEG amplitude works the same way and gets described the same way: peak to peak, bottom of the wave to top. It is worth saying out loud because it means the numbers run roughly double what you would guess if you were measuring from the middle line."
   },
   {
    "h": "Rough sizes worth carrying"
   },
   {
    "p": "Tap the three settings above and watch the microvolt readout, not the picture. An anaesthetised adult on propofol runs somewhere around 100 microvolts peak to peak. Suppression is under 5. Electrocerebral inactivity is 2 or less. Those last two are criteria with their own card further along, so take them as landmarks for now. The two faint horizontal lines are at plus and minus 50 microvolts, so a wave that reaches both of them is 100 peak to peak, and at the other two settings the trace does not come close to either line."
   },
   {
    "h": "The catch, which the card on gain takes up"
   },
   {
    "p": "How big a wave looks on a screen is not the same question as how big it is. The picture depends on a setting, and the setting is not part of the patient."
   }
  ],
  "src": [
   "Guay 2025 — the 5 microvolt suppression and 2 microvolt inactivity anchors the programme teaches."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  "id": "gain",
  "topic": "Describing what you see",
  "title": "Gain and the scale",
  "tldr": "The setting that changes how big a wave looks without changing the patient.",
  "lede": "Turning up the volume does not make the singer louder. It is an honest control and it tells you nothing about the performance.",
  "body": [
   {
    "p": "Try the three settings above. It is one recording, unchanged, drawn three times. At the tightest scale it looks like a patient in trouble and at the widest it looks like a quiet record, and neither impression is about the patient."
   },
   {
    "h": "Sweep speed does the same thing sideways"
   },
   {
    "p": "Widening the stretch of time on screen squeezes the same waves closer together, so a rhythm can look faster or slower than it is. Together the two settings can make one record look like several different ones."
   },
   {
    "h": "So read the number"
   },
   {
    "p": "When size is the actual question, take it off the readout rather than off the picture. This is also the first honest reason to look at the second picture, which does not care what the trace gain was set to."
   }
  ],
  "src": [
   "The scale control here changes only the drawing. The signal behind all three settings is byte-identical."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "Describing what you see",
  "title": "Frequency",
  "tldr": "How fast, counted in cycles per second.",
  "lede": "You do not need an instrument for this one. You need to be able to count, and to know where one second is.",
  "body": [
   {
    "p": "The faint vertical lines on the panel above are one second apart. Count the peaks between two of them and you have the frequency in hertz. Tap through the three settings: two peaks, then six, then ten. Each one is a single oscillation drawn alone, so there is nothing else to count."
   },
   {
    "h": "Frequency and duration are one fact said two ways"
   },
   {
    "p": "A ten hertz rhythm has a cycle lasting a hundred milliseconds. A two hertz rhythm has a cycle lasting five hundred. You can see that above: at 2 Hz each cycle fills half the gap between gridlines, at 10 Hz it fills a tenth. This matters more than it looks, because rhythms get described by their frequency and single waves get described by their duration, and it is the same measurement underneath."
   },
   {
    "h": "Why nobody counts in practice"
   },
   {
    "p": "Because the real record is several rhythms on top of each other and you cannot count a sum by eye. That is the whole reason the second picture exists: it does the counting without pause and shows you the answer as a picture."
   }
  ],
  "src": [
   "The oscillation above is the programme's own single-component band demonstration, generated at 10 Hz."
  ],
  "band": "alpha",
  "kind": "trace",
  "span": 3,
  "scale": 40,
  "ctrl": {
   "k": "Count the peaks between two gridlines",
   "key": "band",
   "def": 2,
   "opts": [
    [
 ...
 },
 {
  ...
  "topic": "Describing what you see",
  "title": "Epoch",
  "tldr": "The slice of time you happen to be looking at, and what it lets you ask.",
  "lede": "A photograph of a crowd and a photograph of a face are both true. They answer different questions and you cannot get one from the other.",
  "body": [
   {
    "p": "An epoch is just the length of record in front of you. Move the control above. Nothing about the patient changes, but the question you are able to ask does, and on this record the change is not subtle: the same trace measures about 105 microvolts peak to peak at four seconds and 140 at sixteen."
   },
   {
    "h": "Short windows answer shape questions"
   },
   {
    "p": "At four seconds you can see individual waves and you could measure one. This is the window for anything with a shape and a name."
   },
   {
    "h": "Long windows answer pattern questions"
   },
   {
    "p": "At sixteen seconds the individual waves are too cramped to measure, but brief events appear that the short window kept missing entirely. They were always there. They just recur more slowly than four seconds, so a four-second view could only ever catch them by luck."
   },
   {
    "h": "Which is why the readout grew"
   },
   {
    "p": "That is the whole reason the peak-to-peak figure changed when you moved the control. It is not a bigger signal. It is a longer look at the same one, and a longer look catches the rare large events."
   },
   {
    "h": "So the honest answer to most disagreements"
   },
   {
    "p": "When two people are looking at the same record and describing it differently, check the window before you check the physiology."
   }
  ],
  "src": [
   "The record behind all three windows is the same, generated continuously. The peak-to-peak figures are measured off it."
  ],
  "cfg": {
   "state": "nrem2"
  },
 ...
 {
  "id": "dsa",
  "topic": "The second picture",
  "title": "The DSA",
  "tldr": "The same signal, redrawn so that minutes fit where the trace shows seconds.",
  "lede": "A thermometer tells you the temperature now. A weather map tells you what has been moving across the country all week. Neither one replaces the other.",
  "body": [
   {
    "p": "The raw trace answers what is happening now. The DSA answers what has been happening. Both come from exactly the same signal."
   },
   {
    "p": "Tap between the two settings above. Nothing about the signal changes; only the way it is drawn. That is the entire claim of this card, and it is worth doing rather than reading."
   },
   {
    "h": "How it is made"
   },
   {
    "p": "Chop the record into short epochs of a few seconds. For each slice, work out how much of the signal sits at each frequency. Draw that slice as a single vertical stripe, with colour standing for how much. Then set the stripes side by side as time passes, so the picture crawls across the screen."
   },
   {
    "h": "The two axes"
   },
   {
    "p": "Time runs across. Frequency runs up, slow at the bottom and fast at the top. Colour is how much power sits at that frequency at that moment. Once you can read those three things you can read any spectrogram on any monitor."
   },
   {
    "h": "What you give up"
   },
   {
    "p": "Shape. Everything the raw trace was good at has been thrown away to buy the long view, which is exactly why every monitor shows you both and why this dictionary teaches both."
   },
   {
    "h": "How much time is on it"
   },
   {
    "p": "The window above holds about 38 seconds, which is short for a density spectral array. A theatre monitor will usually show you many minutes at once, and that compression is the whole reason it exists. The picture does not change in kind when it holds more, only in how much history you can take in at a glance."
   }
  ],
  "src": [
   "Density spectral array is the term the ESAIC consensus uses. Spectrogram is the same object under a different name.",
   "The spectrum here is a sine multitaper estimate, which is the browser-tractable stand-in the programme uses for Chronux DPSS."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "Describing what you see",
  "title": "Morphology",
  "tldr": "The shape of a wave, described before anybody names it.",
  "lede": "You can describe a letter of handwriting without knowing which letter it is. Tall, looped, joined to the next one. The description comes first and it survives being wrong about the letter.",
  "body": [
   {
    "p": "Morphology is that description applied to a wave: how steep, how symmetrical, how sharp, which way up, and does it look like a sine wave or like something else. The element above is drawn alone on a near-silent background so there is nothing else to look at."
   },
   {
    "h": "Sinusoidal, and not"
   },
   {
    "p": "A sine wave is symmetrical: one half is the mirror of the other. The wave above is not. One phase is faster and sharper than the other, and that asymmetry is a property of the physiology rather than of the drawing, which is why it survives being redrawn on any monitor."
   },
   {
    "h": "Which way is up"
   },
   {
    "p": "Clinical EEG is drawn with negative deflections pointing upward, the opposite of most physiological traces, and that is the convention used here. It only matters for a wave like this one: a sine wave looks identical either way, but an asymmetric element turns over. If a record ever looks upside down to you against a textbook, check the convention before you check the patient."
   },
   {
    "h": "Describe first, name second"
   },
   {
    "p": "This is the rule the rest of the dictionary rests on. Two different things can share a name, and one thing can have several names depending on who taught you. A description does not have that problem, and if you can describe it accurately you can always find out later what it is called."
   }
  ],
  "src": [
   "The element above is the programme's slow-wave shape, whose asymmetric down-state is sourced to Guay 2025 (down-states of 0.25 to 2 s).",
   "The negative-up display convention is not stated in any source the programme holds. Confirm how your own equipment draws polarity."
  ],
  "cfg": {
   "state": "quiet",
   "transient": {
 ...
 },
 {
  ...
  "topic": "Describing what you see",
  "title": "Continuity",
  "tldr": "Whether the activity runs on, comes and goes, or arrives as separate events.",
  "lede": "Rain, showers, and a single thunderclap are three different weather reports, and the difference between them is not how hard the water falls.",
  "body": [
   {
    "p": "Continuity is the same distinction applied to a record, and it is one of the few descriptions you can make confidently from across the room. Activity that runs on without interruption is continuous. Activity that stops and restarts is intermittent. A one-off event on an otherwise unremarkable background is episodic."
   },
   {
    "h": "What is on the screen above"
   },
   {
    "p": "That record is intermittent. There are stretches of activity and stretches with almost nothing, alternating. It has a name, the name is a criterion with a number attached, and you will meet it properly further along."
   },
   {
    "h": "Why not just say the name now"
   },
   {
    "p": "Because the useful skill at this stage is being able to say what you see without reaching for a label. A registrar who can say the record went from continuous to intermittent has told you something real and checkable. A registrar who reaches for the name and picks the wrong one has told you nothing, and you cannot tell which has happened."
   }
  ],
  "src": [
   "The record above is generated at a burst-suppression ratio of 0.45 using the programme's own gate."
  ],
  "cfg": {
   "state": "propofol",
   "bsr": 0.45
 ...
 {
  "id": "the-bands",
  "topic": "The frequency bands",
  "title": "The bands",
  "tldr": "Six named slices of the frequency axis, with edges that are conventions rather than facts.",
  "lede": "A graphic equaliser has sliders labelled bass, mid and treble. Nothing in the music changes at the boundary between them. Somebody drew the lines so that people could talk about it.",
  "body": [
   {
    "p": "The EEG frequency axis is divided the same way and for the same reason. Six names, running from the slowest at the bottom to the fastest at the top. Tap through them above and watch the bright stripe climb the axis. That vertical position is the band, and it is the only thing the name refers to."
   },
   {
    "h": "The edges used here"
   },
   {
    "p": "Slow below 1 Hz, delta 1 to 4, theta 4 to 8, alpha 8 to 12, beta 13 to 25, gamma above 25. Carry them as landmarks rather than as thresholds, and do not expect them to add up to a tidy hundred per cent of anything."
   },
   {
    "h": "And they disagree between sources"
   },
   {
    "p": "Guay puts alpha at 8 to 12, which is what you have just read. Purdon puts it at 9 to 12. A recent paediatric paper uses 7.5 to 12.5 and shifts every neighbouring edge to match. Sleep medicine adds a seventh band that anaesthesia does not use at all. None of them is wrong. They are conventions, and which numbers you were taught depends on whose table you learned from."
   },
   {
    "h": "Why that is worth knowing"
   },
   {
    "p": "Because two monitors computing over different edges will not agree, and neither will two people describing the same record. When a band figure is quoted at you, the useful question is which edges it was measured over."
   }
  ],
  "src": [
   "Guay 2025 — the band edges this programme uses.",
   "Purdon 2015 Part I, Table 1 — alpha at 9 to 12 Hz.",
   "Markus et al. Pediatric Anesthesia 2026;36:641-49 — delta 0.5-3.5, theta 3.5-7.5, alpha 7.5-12.5, beta 12.5-30.",
   "<em>Flagged:</em> Guay 2025 and Purdon 2015 are cited second-hand; the primary texts are not held here."
  ],
  "band": "slow",
  "kind": "spec",
  "ctrl": {
   "k": "One band at a time",
   "key": "band",
   "def": 0,
   "opts": [
    [
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Slow",
  "tldr": "Below 1 Hz. The largest and slowest thing on the record.",
  "lede": "Start at the bottom of the ladder. Everything after this is faster and, on the same scale, smaller.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   {
    "h": "Where you see it"
   },
   {
    "p": "It grows more than any other band as anaesthesia deepens, which is most of why a deep record looks like long rolling swells rather than a busy line."
   },
   {
    "h": "A word of warning about the name"
   },
   {
    "p": "Slow here is a claim about speed. There is also a named element called a slow wave, which is a claim about shape. They overlap constantly in conversation and they are not the same claim. That shape gets its own card further along; for now just keep the two claims apart."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
  ],
  "band": "slow",
  "kind": "trace",
  "span": 8,
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Delta",
  "tldr": "1 to 4 Hz. The band that dominates general anaesthesia.",
  "lede": "One step up, and still large enough that it carries most of what you can see on an anaesthetised trace.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   ...
   {
    "p": "Dominant under general anaesthesia and in deep sleep. In infants it dominates the record at baseline, before any drug at all, which is a trap if you are used to adults."
   },
   {
    "h": "Against the card before this one"
   },
   {
    "p": "Slow and delta usually move together and are often spoken of as one thing, slow-delta. That is a convenience rather than a rule, and the two have different edges."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Theta",
  "tldr": "4 to 8 Hz. The middle of the ladder, and easy to overlook.",
  "lede": "Halfway up. The amplitude has fallen by more than half since the slow band, and it keeps falling.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   ...
   {
    "p": "It is sevoflurane's extra stripe against propofol, and it reappears during emergence."
   },
   {
    "h": "Why it gets missed"
   },
   {
    "p": "It sits between two bands that are louder than it in almost every record, so on a raw trace it is buried. This is a band you read off the second picture rather than off the line."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Alpha",
  "tldr": "8 to 12 Hz. The band that moves.",
  "lede": "The most useful band in anaesthesia, and the only one whose location on the head is part of what it means.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   ...
   {
    "p": "Occipital when a patient is awake with their eyes closed. Frontal under a GABA-A anaesthetic. The rhythm does not appear from nowhere; it changes address."
   },
   {
    "h": "Where it is not"
   },
   {
    "p": "In infants it is not reliably present until somewhere between four and six months of age, and the sources do not agree on exactly when."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Beta",
  "tldr": "13 to 25 Hz. Small, fast, and informative at exactly one moment.",
  "lede": "Near the top of the ladder. At this scale you are looking at texture rather than at waves you could measure by eye.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   ...
   {
    "p": "It rises at induction, before consciousness is lost, which is the phenomenon called paradoxical excitation. It also rises in light anaesthesia."
   },
   {
    "h": "Why the size is the point"
   },
   {
    "p": "Beta is roughly a fifth of the amplitude of the slow band. That fall from bottom to top is a real property of the EEG, not a rendering choice, and it is why one fixed vertical scale can never show every band well."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
 ...
 },
 {
  ...
  "topic": "The frequency bands",
  "title": "Gamma",
  "tldr": "Above 25 Hz. The band most likely not to be the brain.",
  "lede": "The top of the ladder, and the one to be most suspicious of.",
  "body": [
   {
    "p": "The trace above is that band on its own, one oscillation with nothing else in the record. On Compare, all six cards use the same window and the same vertical scale, so the size you see is directly comparable between them — but the fast bands pack so many cycles into the window that they read as texture rather than as waves. Close up shortens the window until you can see the individual cycles. Same signal, and the gridline spacing changes with it. The size rises and falls as you watch, which is deliberate: a single frequency held at a fixed amplitude looks like a test tone rather than like a rhythm. That is not the intermittent pattern from Continuity, which drops to almost nothing and comes back."
   },
   ...
   {
    "p": "On a forehead channel a large part of what sits up here is frontalis muscle rather than cortex. Ketamine is the anaesthetic that puts real gamma there."
   },
   {
    "h": "What that means in practice"
   },
   {
    "p": "A number that climbs when a patient is light and their muscles come back is doing something real, but it is not necessarily telling you about the cortex. Anything read off this band deserves the question of whether the patient was paralysed."
   }
  ],
  "src": [
   "Generated from the programme's own single-component band demonstration. The clinical line is the note that demonstration already carries."
 ...
 {
  "id": "slow-wave",
  "topic": "The waves themselves",
  "title": "Slow wave",
  "tldr": "One cycle of the slow rhythm, on its own, measured by its down-state.",
  "lede": "You have already met this one. It was the shape on the morphology card, and it is worth naming now that you have the vocabulary for it.",
  "body": [
   {
    "p": "A single cycle of the slow-delta rhythm, drawn alone on a near-silent background of about 2 microvolts so there is nothing else to look at. It peaks at about 91 microvolts here."
   },
   {
    "h": "It is measured by its down-state, not its whole cycle"
   },
   {
    "p": "The figure is 453 milliseconds, and that is the down-state alone. Guay gives down-states of a quarter of a second to two seconds. Measuring the whole cycle would give a different and less useful number, because the down-state is the part that carries the meaning."
   },
   {
    "h": "Why the asymmetry matters"
   },
   {
    "p": "The down-state is sharper than the up-state. Tap the polarity control and watch the whole thing turn over. That is what the negative-up convention does to an asymmetric element, and it is the only reason the convention is worth knowing."
   },
   {
    "h": "Slow wave against delta wave"
   },
   {
    "p": "A delta wave is any wave in the delta band, which is a claim about speed. A slow wave is this shape, which is a claim about form. They get used interchangeably in conversation and they are not the same claim."
   }
  ],
  "src": [
   "Guay 2025 — down-states of 0.25 to 2 s. This is the only one of the four named elements here whose criterion comes from a source the programme holds."
  ],
  "cfg": {
   "state": "quiet",
   "transient": {
 ...
 },
 {
  ...
  "topic": "The waves themselves",
  "title": "K-complex",
  "tldr": "A large, long, two-part deflection, biggest at the front of the head.",
  "lede": "The largest single thing you will see on a normal record, and the easiest to recognise once you have seen one.",
  "body": [
   {
    "p": "A sharp negative component followed by a slower positive one, frontally maximal, and long. It measures 891 milliseconds here and peaks at about 117 microvolts, which is more than the whole propofol record you met earlier."
   },
   {
    "h": "Long is the defining feature"
   },
   {
    "p": "The usual criterion is at least half a second. That figure is an AASM scoring definition, which is licensed and is not held by this programme, so treat it as orientation rather than as something you can quote."
   },
   {
    "h": "Two parts, and both of them count"
   },
   {
    "p": "Tap the polarity control. The sharp phase and the slow phase swap sides, and the shape stops looking like the one in your textbook if the textbook drew it the other way up. Nothing about the element changed."
   }
  ],
  "src": [
   "<em>Flagged:</em> the at-least-0.5-second criterion is an AASM scoring definition. AASM is licensed and is not held by this programme. Confirm it against your own reference before teaching it."
  ],
  "cfg": {
   "state": "quiet",
   "transient": {
 ...
 },
 {
  "id": "vertex",
  "topic": "The waves themselves",
  "title": "Vertex sharp wave",
  "tldr": "A single sharp deflection, about six times shorter than a K-complex.",
  "lede": "Sharp means short. It does not mean steeply drawn, and the difference is the whole point of this card.",
  "body": [
   {
    "p": "One negative transient, measuring 141 milliseconds against the K-complex's 891, and peaking at about 78 microvolts."
   },
   {
    "h": "Sharpness is a duration, not an impression"
   },
   {
    "p": "Go back to the card on gain and remember what changing the vertical scale did. Steepness on a screen is a setting. Duration is not. That is why sharpness is something you measure rather than something you judge, and it is why the two elements are told apart by one number."
   },
   {
    "h": "What the polarity control shows here"
   },
   {
    "p": "Less than on the other two, because this shape is nearly symmetrical about its peak. Which is itself the lesson: the convention only bites on an asymmetric element."
   }
  ],
  "src": [
   "<em>Flagged:</em> the duration range for this element is clinical convention and is not stated in any source the programme holds."
  ],
  "cfg": {
   "state": "quiet",
   "transient": {
 ...
 },
 {
  ...
  "topic": "The waves themselves",
  "title": "Sleep spindle",
  "tldr": "Not a single deflection at all, but a short run of oscillation.",
  "lede": "Put this beside a K-complex and the difference between an element and a rhythm stops being abstract.",
  "body": [
   {
    "p": "A run of oscillation between about 11 and 16 hertz, lasting around 805 milliseconds here and peaking at only 34 microvolts. Conventionally a run has to last at least half a second to count."
   },
   {
    "h": "One event, many cycles"
   },
   {
    "p": "A K-complex is one deflection that happens to be long. This is many cycles inside one envelope that begins and ends. Both last about eight hundred milliseconds, and they are nothing like each other."
   },
   {
    "h": "Why it is small"
   },
   {
    "p": "At 34 microvolts it is a third the size of a K-complex. It sits in the sigma range just above alpha, and by the ladder you have already walked, faster means smaller."
   },
   {
    "h": "Polarity does almost nothing here"
   },
   {
    "p": "An oscillation is symmetrical, so turning it over leaves it looking the same. Compare that with the slow wave and you have the whole of why the convention matters where it matters."
   }
  ],
  "src": [
   "<em>Flagged:</em> the 11 to 16 hertz range and the half-second minimum are scoring conventions and are not stated in any source the programme holds."
  ],
  "cfg": {
   "state": "quiet",
   "transient": {
 ...
 },
 {
  ...
  "topic": "The waves themselves",
  "title": "Suppression",
  "tldr": "Three words that everything later depends on, separated by one number.",
  "lede": "These are not descriptions of how a record feels. They are three names for three measurements, and getting them the wrong way round changes what you have said.",
  "body": [
   {
    "p": "A burst is a segment of activity. Suppression is the near-flat segment between bursts, under 5 microvolts peak to peak. Attenuation is a fall in amplitude that does not reach that criterion. Both a burst and a suppression are on the record above."
   },
   {
    "h": "The scale control is the argument"
   },
   {
    "p": "Move it. At the widest setting the quiet stretch looks completely flat and the bursts look modest. At the tightest it looks like there is something going on in both. Neither impression is a measurement, and the microvolt readout has not moved through any of it."
   },
   {
    "h": "So the difference between the three words is a number"
   },
   {
    "p": "Not an impression, and not how flat something looks at whatever gain the last person left the monitor on. If you cannot say which side of 5 microvolts a segment falls, you cannot yet say which of the three words applies."
   }
  ],
  "src": [
   "Guay 2025 — the 5 microvolt suppression criterion, and attenuation as a fall that does not reach it."
  ],
  "cfg": {
   "state": "propofol",
   "bsr": 0.45
 ...
 },
 {
  ...
  "topic": "Rhythms",
  "title": "Frontal alpha",
  "tldr": "A steady rhythm over the forehead that turns up when propofol takes hold.",
  "lede": "Pick up a landline and you get a dial tone. It tells you nothing about who you are calling, only that the line is open and the exchange is working.",
  "body": [
   {
    "p": "Frontal alpha works a little like that. Once propofol has taken an adult past loss of consciousness, a rhythm of about ten cycles a second appears over the forehead, riding on much slower and much larger waves. On the record above the slow and delta bands carry about 89% of the power and the alpha about 11%, which is why it reads as a fine ripple on a long swell rather than as a wave in its own right."
   },
   {
    "h": "The rhythm changed address, it did not appear"
   },
   {
    "p": "An awake patient with their eyes shut already has alpha. It sits at the back of the head. Under propofol it turns up at the front instead. That relocation is the finding, and it reverses on the way out."
   },
   {
    "h": "What it is good for"
   },
   {
    "p": "It is one of the steadier things on the screen once you can see it. It arrives with loss of consciousness and it leaves on emergence. If it fades in the middle of a case while nothing about the dose has changed, that is worth a second look at the patient rather than at the monitor."
   },
   {
    "h": "Where it is not"
   },
   {
    "p": "Tap the second setting. Same drug, same dose, a two-month-old, and the alpha is simply not there. It is not reliably present until somewhere between four and six months of age, and the sources do not agree on exactly when. In a small baby its absence tells you about the age of the brain, not about the depth of anaesthesia."
   }
  ],
  "src": [
   "Purdon et al. Proc Natl Acad Sci USA 2013;110(12):E1142-51. doi 10.1073/pnas.1221180110 — the anterior shift.",
   "Bong &amp; Yuan. Anesth Analg 2026;142:1155-68, and Markus et al. Pediatric Anesthesia 2026;36:641-49 — the paediatric threshold, and the disagreement about it.",
   "<em>Flagged:</em> the Purdon primary text is cited second-hand in the programme's own notes. Verify before teaching from it."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "title": "Where the sensor sits",
  "tldr": "The strip on the forehead records the front of the head, and the alpha you were taught lives at the back.",
  "lede": "A rain gauge in the front garden measures the front garden. If the shower fell on the back garden, the gauge reads dry and it has not lied.",
  "body": [
   {
    "p": "Every monitoring strip sits on the forehead; the one in the source papers records from four spots near Fp1, Fp2, F7 and F8. Awake with the eyes closed, the alpha rhythm is at the back of the head. Tap between the two awake settings above: on the same record the forehead carries about one seventh of the alpha power that the back of the head does, so a frontal monitor on an awake patient shows very little alpha, and the reason is where the sensor sits."
   },
   {
    "h": "Under propofol the address changes"
   },
   {
    "p": "Now tap the two propofol settings. The occipital alpha has faded and a frontal alpha has appeared, about fourteen times stronger at the front than at the back. Purdon's group called this anteriorisation. From the forehead you only ever see the rhythm arrive, because the strip never recorded the back of the head, so the move the textbooks describe happens off screen."
   },
   {
    "h": "What to expect on a frontal monitor"
   },
   {
    "p": "Alpha there is a frontal fact. Expect little of it awake, expect it to appear with loss of consciousness under propofol or sevoflurane, and expect it to go on emergence. If someone says the awake patient has no alpha, ask where the sensor was."
   }
  ],
  "src": [
   "Purdon 2015 Part I, p. 947 - most brain monitors record a few channels from the front of the head; eyes-closed alpha is occipital in the awake patient; anteriorisation with loss of consciousness and its reversal on emergence. Electrode positions from their methods, the Sedline array at approximately Fp1, Fp2, F7 and F8.",
   "Guay 2025 Part 2, p. 1598-9 - alpha most prominent occipitally in the resting, eyes-closed state; under propofol and sevoflurane occipital alpha power falls as frontal alpha power rises.",
   "The one-seventh and fourteen-fold figures are alpha band power measured off the programme's own two channels, whose weighting follows Purdon 2013's anterior shift. The back channel is a teaching model; no occipital recording was used to build it."
  ],
  "cfg": {
   "state": "awake",
   "ch": 1
 ...
 },
 {
  ...
  "tldr": "The cortex goes quiet between bursts. That is the whole of what it tells you.",
  "lede": "A smoke alarm tells you there is smoke. It will not tell you whether that is toast, a candle or the wiring.",
  "body": [
   {
    "p": "It is a good instrument for exactly that reason: completely unambiguous about one thing, completely silent about everything else. Burst suppression carries the same shape of information. The record alternates. A few seconds of activity, then a stretch that is almost flat, then activity again. The flat stretches are the suppression."
   },
   {
    "h": "How flat counts as flat"
   },
   {
    "p": "The threshold taught here is under 5 microvolts peak to peak. On the middle setting the suppressed stretches measure about 1.7 against bursts of 96, so the difference stops being subtle once you know to look for it. Critical care and the ACNS glossary use under 10 microvolts for the same word. Two numbers for one term is worth knowing before somebody quotes one of them at you."
   },
   {
    "h": "The third setting is not more of the same"
   },
   {
    "p": "Tap it and the bursts stop entirely. A record with no bursts at all, measuring 2 microvolts or less, is electrocerebral inactivity, and that is a different statement. Notice what does not distinguish them: the suppressed stretches are about as small in both. What makes it burst suppression is that something still bursts."
   },
   {
    "h": "It is a state, not a cause"
   },
   {
    "p": "Too much anaesthetic will produce it. So will hypothermia, cerebral ischaemia, and the period after a seizure. The record cannot tell you which of those you are looking at. That is not a gap to work around, it is how the finding is meant to be used. You read the trace, then you read the temperature, the pressure, the dose and the patient."
   }
  ],
  "src": [
   "Guay 2025 — the 5 microvolt suppression figure and the 2 microvolt inactivity figure this programme teaches.",
   "Sewell, Abbas &amp; Kane. BJA Education 2019;19(3):74-82. doi 10.1016/j.bjae.2018.11.002 — the 10 microvolt critical-care figure, and the alternative causes."
  ],
  "cfg": {
   "state": "propofol",
   "bsr": 0.6
 ...
 },
 {
  ...
  "tldr": "The share of the recent record that was suppressed, shown as a percentage.",
  "lede": "A parking meter that reports how much of the last hour the space was occupied. Forty per cent is a share of time, and it says nothing about which car was there or when.",
  "body": [
   {
    "p": "The ratio takes the criterion you already know, suppression under 5 microvolts, and asks how much of a window of record fell below it. The answer is a fraction of time. Move the depth control and read the figure in the corner of the trace: mostly bursts reads around 25 to 30 per cent, mostly suppressed around 55, and with no bursts at all it reads 100. The figure drifts as the window slides along the record, because each new second replaces an old one."
   },
   {
    "h": "The window is part of the number"
   },
   {
    "p": "On the BIS monitor the ratio is the share of the last 63 seconds that was suppressed, so 5 per cent means three seconds of suppression in the last minute. A number averaged over a minute moves slowly, which is why suppression shows on the trace first and reaches the figure later. The readout on this card averages about the last half minute in half-second pieces, so it moves faster than a monitor would."
   },
   {
    "h": "What it cannot tell you"
   },
   {
    "p": "The number counts time below a threshold and has no way of knowing why the record went quiet. It reads the same whether the suppression came from the anaesthetic, from cooling, from low blood sugar or from a brain short of blood or oxygen. Studies that set the monitor's figure against a person reading the same trace found the monitor reported less suppression than was there, so the ratio is a prompt to look at the trace, and the trace settles it."
   }
  ],
  "src": [
   "Purdon 2015 Part I, p. 948 and Figure 8 - the burst suppression ratio as the fraction of a time interval in which the record is suppressed, 0 to 1; the plus and minus 5 microvolt thresholds separating bursts from suppression.",
   "Aydin et al. Anesth Analg 2026;142:1056-66. doi 10.1213/ANE.0000000000007877 - the BIS ratio is the percentage of the last 63 seconds fully suppressed; 5 per cent equals 3 seconds; the ratio rose with a lag behind visually identified suppression; no agreed threshold for a clinically relevant ratio.",
   "Garcia and Goldstein, Anesth Analg 2026;142:1050-54. doi 10.1213/ANE.0000000000007934 - algorithm-based suppression ratios underestimate suppression against visual inspection of the raw EEG.",
   "Guay 2025 Part 2, p. 1600 - burst suppression precipitated by ischaemia, hypoxaemia, hypothermia, hypoglycaemia and brain injury as well as by the anaesthetic.",
   "The readout figures are measured from the programme's own gate: half-second pieces of about the last half minute under 5 microvolts peak to peak."
  ],
  "cfg": {
   "state": "propofol",
   "bsr": 0.6
 ...
 },
 {
  ...
  "topic": "The numbers",
  "title": "SEF95",
  "tldr": "One line across the spectrogram with 95% of the power below it.",
  "lede": "Look at a loaded ship and you can see where the water comes up the hull. The waterline tells you nothing at all about what is in the hold.",
  "body": [
   {
    "p": "SEF95 is a waterline drawn across the frequency axis. Take all the power in the record, from the slowest waves to the fastest, and find the frequency with 95% of it underneath. The picture above is the density spectral array you already know, with that line drawn on it. This is how a monitor shows it, and it is worth noticing why: the line and the picture it came from are on the same screen."
   },
   {
    "h": "Why it moves the way it does"
   },
   {
    "p": "Slow, large waves carry most of the power. So as a patient goes deeper and the record fills with slow activity, the line drops. As they lighten and faster activity comes back, it climbs. As orientation rather than as targets: around 20 Hz at emergence, around 17 when light, around 10 when deep. The record above sits at about 10.3."
   },
   {
    "h": "The catch"
   },
   {
    "p": "It is a summary, and summarising costs you the picture. On this exact record 95% of the power sits below 10.3 Hz, but 50% of it sits below 1.0 Hz. Same signal, two lines nine hertz apart. Which of them you are shown is a choice somebody else made, and so is the frequency band it was measured over, which here is 0.5 to 45 Hz."
   },
   {
    "h": "It is not the number on your monitor"
   },
   {
    "p": "BIS, PSi and Narcotrend are proprietary and are calculated differently. SEF95 is one of the few figures you can work out openly and check against the picture underneath it."
   }
  ],
  "src": [
   "Bong &amp; Yuan. Anesth Analg 2026;142:1155-68. doi 10.1213/ANE.0000000000007230 — the orientation values.",
   "<em>Provenance:</em> that is a narrative review rather than a trial, and one author declares a Masimo consulting fee and research grant."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "The numbers",
  "title": "The index",
  "tldr": "The single number on your monitor, computed from this same signal by a method the maker does not publish.",
  "lede": "A credit score. It is built from real transactions by a formula you cannot see, and two people with very different accounts can carry the same number.",
  "body": [
   {
    "p": "BIS, PSi and the Narcotrend index are numbers from 0 to 100 built from the frontal EEG on the same strip, with lower meaning deeper. The formulas are proprietary, which is why this programme shows you SEF95 and never shows an index: any index drawn here would have to be invented. The record on this card is the record such a number would be made from."
   },
   {
    "h": "When the index reads adequate during suppression"
   },
   {
    "p": "Tap the second setting. Suppression has crept into the record, and the ratio in the corner of the trace has risen to around 15 to 20 per cent. In 62 patients aged around 81 having valve replacement, titrated to a BIS of 40 to 60, 29 had a run of at least ten seconds in which the index stayed inside that range while the suppression ratio sat at 5 per cent or more. The median run was 52 seconds and the longest 202. The index read adequate while the record underneath it was suppressed."
   },
   {
    "h": "So read the record"
   },
   {
    "p": "An index is a summary of the spectrum you can already see, and where the two disagree the record is the observation. The editorial published with that study says to reduce the hypnotic when suppression appears even if the index drifts into the 60s. Treat the number as a prompt to look at the trace, because two states this programme can tell apart from the display can share one index value."
   }
  ],
  "src": [
   "Purdon 2015 Part I, p. 938 - indices scaled 0 to 100 with low values meaning greater depth; the algorithms in many monitors are proprietary; the indices assume one value means the same depth for every anaesthetic.",
   "Aydin et al. Anesth Analg 2026;142:1056-66. doi 10.1213/ANE.0000000000007877 - retrospective, 62 patients aged 80.9 ± 5.8 years, elective transcatheter aortic valve replacement, sevoflurane with remifentanil; 29 of 62 with BIS 40 to 60 and a suppression ratio of 5 per cent or more for over ten seconds, median run 52 s, maximum 202 s. Kreuzer and Schneider hold EEG patents; Kreuzer reports funding from Masimo, Narcotrend, Medtronic and Fresenius Kabi.",
   "Garcia and Goldstein, Anesth Analg 2026;142:1050-54. doi 10.1213/ANE.0000000000007934 - editorial: reduce hypnotic dosing when burst suppression emerges even if the BIS drifts into the 60s; reading the raw EEG as integral to training. Goldstein handled the Aydin manuscript as editor; both authors declare EEG-related patents and company links.",
   "The ratio in the corner of the trace is the programme's own measured figure, described on the Burst suppression ratio card."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "Artefacts",
  "title": "Artefact",
  "tldr": "Signal that was never the brain.",
  "lede": "A microphone picks up the singer and also the traffic outside. The traffic is on the tape; it was never part of the song.",
  "body": [
   {
    "p": "The electrode records everything electrical that reaches it, and the brain is only one of the sources. Some of the rest comes from equipment and surroundings, which the sources call technical; some comes from the patient's own body, which they call physiological. The rule the app already teaches carries the whole card: technical artefacts have shapes physiology cannot make, and physiological artefacts come from muscle and eye, with their own topography."
   }
  ],
  "src": [
   "AR1, AR2, AR5, Berger-Estilita 2026, mapped to module 05."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "Artefacts",
  "title": "Muscle",
  "tldr": "The forehead has muscles, and they are loud.",
  "lede": "Fingers drumming on the microphone stand. It arrives with the music, and it stops the moment the fingers do.",
  "body": [
   {
    "p": "Frontalis muscle activity writes fast, spiky voltage on top of whatever the brain is doing, loading the top of the display while the record underneath is unchanged. It rides hardest on frontal electrodes, which is exactly where the monitoring strip sits."
   }
  ],
  "src": [
   "Suppressed by neuromuscular blockade, which is why it moves a processed index without the brain changing."
  ],
  "cfg": {
   "state": "propofol"
  },
 ...
 },
 {
  ...
  "topic": "Artefacts",
  "title": "Eye blink",
  "tldr": "The biggest thing on a frontal trace can be the eyes.",
  "lede": "A wiper sweeping across a dashcam lens - huge in the picture, nothing to do with the road.",
  "body": [
   {
    "p": "A blink writes a tall, slow dome lasting about a third of a second, far larger than the rhythms behind it. It is big, brief, stereotyped and repeating, which is how you tell it from a slow wave, and the engine draws it at about a third of its size on the back channel - the topography is a text claim here, since the primer window is frontal-only."
   }
  ],
  "src": [
   "Blinks and eye movement: large, slow, frontal. PHYSIOLOGICAL."
 ...
 },
 {
  ...
  "tldr": "The wall socket, drawn on the trace.",
  "lede": "The feedback whine from a PA system - it holds a single pitch no instrument holds.",
  "body": [
   {
    "p": "Mains interference arrives at exactly one frequency, 50 or 60 Hz depending on the country. Brain rhythms occupy bands several hertz wide, so a perfectly thin, perfectly steady oscillation at one frequency has a machine behind it. On this display the line sits above the 40 Hz top of the spectral array, so you find mains on the trace and in the quality figure."
   }
  ],
  "src": [
   "Mains interference: a single narrow line. TECHNICAL."
 ...
 },
 {
  ...
  "topic": "Artefacts",
  "title": "Electrode pop",
  "tldr": "Physiology has no jumps.",
  "lede": "A half-seated headphone plug. The music jumps when the plug moves, and the jump tells you about the plug.",
  "body": [
   {
    "p": "A failing electrode contact writes an abrupt step that then decays away, with a slow wander underneath. No brain process changes voltage in an instant, and the excursions run past 150 microvolts, beyond anything an anaesthetised cortex produces - both giveaways point at the contact."
   }
  ],
  "src": [
   "Electrode pop / movement: abrupt step with exponential decay plus drift. TECHNICAL."
  ],
  "cfg": {
   "state": "propofol"
  },
...
/* ==========================================================================
   THE PRIMER — a basics on-ramp that runs before module 01.

   Everything here is ADDITIVE. It calls the app's own makeEngine, spectrum,
   sefBand, jet, BAND_DEMO, TRANSIENTS and P, so there is one engine and one
   theme table, not two.

   Written against the subset mini.js implements: useState / useEffect / useRef.
   No fragments, no context, and NO MEMOISATION OF ANY KIND — the build already
   rejects that, because P is mutated in place and a memoised subtree would stop
   seeing a theme change.
   ========================================================================== */

/* ---- scopes ---------------------------------------------------------------
   SCOPE is a singleton for the ONE instrument the app shows. The primer's index
   shows 31 at once, so it needs a map. Kept SEPARATE and additive: nothing here
   touches SCOPE, and SCOPE never sees these buffers.

   Held OUTSIDE the components for the same reason SCOPE is: a tab change would
   otherwise unmount every canvas and throw away its history.                  */

const PSCOPES = {};
const PSCOPE_MAX = 40;
const PSCOPE_ORDER = [];
const PCOL_S = 0.25;                 // seconds per spectrogram column
const PNCOL = 150;
const PNBIN = Math.round(40 / DF);

function pOrdered(s) {
  if (!s.scratch || s.scratch.length !== s.buf.length) s.scratch = new Float64Array(s.buf.length);
  const out = s.scratch, n = s.buf.length, head = n - s.i;
  out.set(s.buf.subarray(s.i));
  out.set(s.buf.subarray(0, s.i), head);
  return out;
}

function pScopeFor(key, cfg, span, wantsDSA, hold = 0) {
  let s = PSCOPES[key];
  if (s) return s;
  // hold: seconds of record to keep beyond what the window shows, so a figure
  // measured over the buffer (the burst suppression ratio) has a longer, steadier
  // window than the trace it sits on. The trace still draws only the last span.
  const need = Math.max(Math.round(Math.max(span, hold) * FS), FS * 4);
  // ch: 0 is the forehead, 1 the back of the head. The engine already draws both;
  // the primer only ever asked it for the front until the sensor card.
  const ch = cfg.ch ?? 0;
  s = { eng: makeEngine(3), cfg, ch, span, buf: new Float64Array(need), i: 0, t: 0, acc: 0, cols: [], dsa: wantsDSA };
  // prime the ring buffer, then the column history, through the SAME path the
  // live loop uses, so a DSA never opens as a block in the corner
  for (let k = 0; k < need; k++) { s.buf[k] = s.eng(s.t, cfg, ch); s.t += 1 / FS; }
  if (wantsDSA) {
    const per = Math.round(PCOL_S * FS);
    for (let c = 0; c < PNCOL; c++) {
      for (let j = 0; j < per; j++) {
        s.buf[s.i] = s.eng(s.t, cfg, ch);
        s.i = (s.i + 1) % s.buf.length; s.t += 1 / FS;
      }
      s.cols.push(spectrum(pOrdered(s).subarray(s.buf.length - WIN_N), NFFT, 3));
    }
  }
  PSCOPES[key] = s;
  PSCOPE_ORDER.push(key);
  while (PSCOPE_ORDER.length > PSCOPE_MAX) {
    const k = PSCOPE_ORDER.shift();
    delete PSCOPES[k];
  }
  return s;
}

function pAdvance(s, dt) {
  const n = Math.min(Math.round(dt * FS), FS);       // cap after a background tab
  for (let k = 0; k < n; k++) {
    s.buf[s.i] = s.eng(s.t, s.cfg, s.ch || 0);
    s.i = (s.i + 1) % s.buf.length; s.t += 1 / FS;
  }
  if (!s.dsa || !n) return;
  s.acc += n / FS;
  while (s.acc >= PCOL_S) {
    s.acc -= PCOL_S;
    s.cols.push(spectrum(pOrdered(s).subarray(s.buf.length - WIN_N), NFFT, 3));
    if (s.cols.length > PNCOL) s.cols.shift();
  }
}

/* ---- the burst suppression ratio of a held buffer --------------------------
   measuredBSR counts half-second pieces from the start of whatever it is given.
   Counting from the start of a sliding buffer moves the grid under the record on
   every sample, and the count then flickers as burst edges cross piece boundaries.
   Anchoring the pieces to the record's own clock makes the figure change only
   when the record does.                                                          */

function pBSR(s) {
  const sig = pOrdered(s), w = Math.round(0.5 * FS);
  const first = Math.round(s.t * FS) - sig.length;   // absolute index of sig[0]
  const off = (w - (first % w)) % w;
  return measuredBSR(sig.subarray(off));
}

/* ---- resolving a card to something the engine can draw -------------------- */

function pResolve(card, optIdx) {
  const c = { band: card.band, cfg: card.cfg, kind: card.kind, scale: card.scale,
              span: card.span, sef: card.sef, pol: 1 };
  let tag = "";
  if (card.ctrl) {
    const v = card.ctrl.opts[optIdx][0];
    tag = ":" + optIdx;
    if (card.ctrl.key === "scale") c.scale = v;
    else if (card.ctrl.key === "span") c.span = v;
    else if (card.ctrl.key === "band") c.band = v;
    else if (card.ctrl.key === "view") c.kind = v;
    else if (card.ctrl.key === "cfg") c.cfg = v;
    else if (card.ctrl.key === "pol") c.pol = v;
  }
  const cfg = c.band ? { custom: BAND_DEMO[c.band] } : (c.cfg || { state: "propofol" });
  return {
    key: card.id + tag + ":" + (c.span || 8) + ":" + (c.band || ""),
    cfg, kind: c.kind || "trace", scale: c.scale || 70, span: c.span || 8,
    sef: !!c.sef, pol: c.pol === -1 ? -1 : 1,
    dsa: c.kind === "spec" || (card.ctrl && card.ctrl.key === "view"),
    // the burst suppression ratio is measured over 32 s of held record, eight
    // gate periods, so the figure moves less than it would over the 8 s shown
    bsr: card.readout === "bsr", hold: card.readout === "bsr" ? 32 : 0,
  };
}

const pGrid = (span) => (span >= 2 ? 1 : 0.1);
const pGridLabel = (span) => (pGrid(span) === 1 ? "gridlines 1 s apart" : "gridlines 100 ms apart");

/* ---- one instrument window ------------------------------------------------
   Colours read P on every frame, so a theme change lands on the next frame
   without remounting — the same property that keeps SCOPE's spectrogram alive.
   The window itself stays specBg in BOTH themes: it sits inside a P.surface
   card, so a P.surface window would be the same colour as its own card.       */


function PrimerScope({ spec, height, small }) {
  const ref = useRef(null);
  const specRef = useRef(spec);
  const lastRef = useRef(0);
  const [ro, setRo] = useState({ pp: 0, sef: 0 });
  ...
  useEffect(() => {
    ...
    let ctx = fit();
    const onResize = () => { ctx = fit(); };
    window.addEventListener("resize", onResize);
    const reduced = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const minGap = reduced ? 1000 / 12 : 1000 / 30;

    const step = (now) => {
      raf = requestAnimationFrame(step);
      if (!ctx) return;
      if (lastRef.current && now - lastRef.current < minGap) return;
      const dt = lastRef.current ? Math.min((now - lastRef.current) / 1000, 0.5) : 0;
      lastRef.current = now;

      const sp = specRef.current;
      const s = pScopeFor(sp.key, sp.cfg, sp.span, sp.dsa, sp.hold);
      if (dt) pAdvance(s, dt);
      const sig = pOrdered(s);
      const w = cv.width / dpr, h = cv.height / dpr;

      if (sp.kind === "spec") {
        if (!off) {
          off = document.createElement("canvas");
          off.width = PNCOL; off.height = PNBIN;
          octx = off.getContext("2d");
          if (!octx) return;
          img = octx.createImageData(PNCOL, PNBIN);
        }
        const cols = s.cols, d = img.data, sb = rgbOf(P.specBg);
        for (let cx = 0; cx < PNCOL; cx++) {
          const col = cols[cols.length - PNCOL + cx];
          for (let y = 0; y < PNBIN; y++) {
            const o = (y * PNCOL + cx) * 4;
            if (!col) { d[o] = sb[0]; d[o + 1] = sb[1]; d[o + 2] = sb[2]; d[o + 3] = 255; continue; }
            const db = 10 * Math.log10(Math.max(1e-6, col[PNBIN - 1 - y]));
            const rgb = jet((db + 8) / 52);
            d[o] = rgb[0]; d[o + 1] = rgb[1]; d[o + 2] = rgb[2]; d[o + 3] = 255;
          }
        }
        octx.putImageData(img, 0, 0);
        const padL = small ? 0 : 26, padB = small ? 0 : 14;
        ctx.fillStyle = P.specBg; ctx.fillRect(0, 0, w, h);
        ctx.drawImage(off, padL, 0, w - padL, h - padB);
        const last = s.cols[s.cols.length - 1];
        const s95 = last ? sefBand(last, 0.95, NFFT, FS, 45) : 0;
        if (sp.sef && last) {
          const sy = (h - padB) * (1 - Math.min(s95, 40) / 40);
          ctx.strokeStyle = P.specInk; ctx.lineWidth = 1.6;
          ctx.beginPath(); ctx.moveTo(padL, sy); ctx.lineTo(w, sy); ctx.stroke();
          if (!small) {
            ctx.fillStyle = P.specInk; ctx.font = "600 10px " + MN; ctx.textAlign = "right";
            ctx.fillText("SEF95 " + s95.toFixed(1) + " Hz", w - 5, Math.max(11, sy - 5));
          }
        }
        if (!small) {
          ctx.fillStyle = tint(P.specInk, 0.55); ctx.font = "9px " + MN; ctx.textAlign = "right";
          for (let i = 0; i <= 40; i += 10) {
            const yy = (h - padB) * (1 - i / 40);
            ctx.fillText(i === 40 ? "40 Hz" : String(i), padL - 4, Math.min(h - padB - 1, Math.max(8, yy + 3)));
          }
          ctx.textAlign = "left";
          ctx.fillText("\u2190 " + Math.round(PNCOL * PCOL_S) + " s of record", padL + 2, h - 4);
        }
        if (Math.abs(s95 - ro.sef) > 0.05) setRo({ pp: ro.pp, sef: s95 });
        return;
      }

      // trace
      const scale = sp.scale, span = sp.span, pol = sp.pol;
      const n = Math.min(sig.length, Math.round(span * FS)), start = sig.length - n;
      ctx.fillStyle = P.specBg; ctx.fillRect(0, 0, w, h);
      ctx.strokeStyle = tint(P.specInk, 0.10); ctx.lineWidth = 1;
      const gi = pGrid(span);
      for (let g = gi; g < span - 1e-9; g += gi) {
        const px = Math.round(w * g / span) + 0.5;
        ctx.beginPath(); ctx.moveTo(px, 0); ctx.lineTo(px, h); ctx.stroke();
      }
      const mid = h / 2, k = (h / 2 - 3) / scale;
      if (scale > 55 && h > 60) {
        ctx.strokeStyle = tint(P.specRef, 0.26);
        for (let u = 0; u < 2; u++) {
          const yy = mid - (u ? 50 : -50) * k;
          if (yy > 2 && yy < h - 2) { ctx.beginPath(); ctx.moveTo(0, yy); ctx.lineTo(w, yy); ctx.stroke(); }
        }
      }
      ctx.strokeStyle = tint(P.specInk, 0.16);
      ctx.beginPath(); ctx.moveTo(0, Math.round(mid) + 0.5); ctx.lineTo(w, Math.round(mid) + 0.5); ctx.stroke();
      ctx.strokeStyle = P.specInk; ctx.lineWidth = h < 70 ? 1 : 1.4; ctx.lineJoin = "round";
      ctx.beginPath();
      // Catmull-Rom when stretched: at a close-up window there are four samples
      // per cycle at 32 Hz, and a polyline through four points under-reads the
      // true peak by up to 29%.
      const sub = (w / Math.max(1, n - 1)) > 3 ? 6 : 1;
      const g0 = (j) => sig[start + Math.max(0, Math.min(n - 1, j))];
      let lo = 1e9, hi = -1e9;
      for (let i = 0; i < n; i++) { const v = g0(i); if (v < lo) lo = v; if (v > hi) hi = v; }
      for (let i = 0; i < n - 1; i++) {
        for (let q = 0; q < sub; q++) {
          const u = q / sub, u2 = u * u, u3 = u2 * u;
          const v = sub === 1 ? g0(i)
            : 0.5 * ((2 * g0(i)) + (-g0(i - 1) + g0(i + 1)) * u
                   + (2 * g0(i - 1) - 5 * g0(i) + 4 * g0(i + 1) - g0(i + 2)) * u2
                   + (-g0(i - 1) + 3 * g0(i) - 3 * g0(i + 1) + g0(i + 2)) * u3);
          const px = ((i + u) / (n - 1)) * w, y = mid + pol * v * k;
          if (i === 0 && q === 0) ctx.moveTo(px, y); else ctx.lineTo(px, y);
        }
      }
      ctx.lineTo(w, mid + pol * g0(n - 1) * k);
      ctx.stroke();
      if (sp.bsr && !small) {
        // measured over the whole held buffer, 0.5 s pieces under 5 uV peak to
        // peak, the same measuredBSR the app's own readout uses, on pieces
        // anchored to the record's clock. A backing pill keeps the figure
        // legible when a burst runs under it.
        const label = "BSR " + Math.round(pBSR(s) * 100) + "%";
        ctx.font = "600 10px " + MN; ctx.textAlign = "right";
        const tw = ctx.measureText(label).width;
        ctx.fillStyle = tint(P.specBg, 0.82); ctx.fillRect(w - 5 - tw - 6, 2, tw + 10, 15);
        ctx.fillStyle = P.specInk; ctx.fillText(label, w - 6, 13);
      }
      const pp = hi - lo;
      if (Math.abs(pp - ro.pp) > 0.5) setRo({ pp, sef: ro.sef });
    };
    raf = requestAnimationFrame(step);
    return () => { cancelAnimationFrame(raf); window.removeEventListener("resize", onResize); };
  }, []);

  const label = spec.kind === "spec"
    ? "Density spectral array, 0 to 40 hertz, " + Math.round(PNCOL * PCOL_S) + " seconds of record, colour is power"
    : "Live EEG trace, " + spec.span + " second window";
  return (
    <div style={{ background: P.specBg, borderRadius: small ? 8 : 10, overflow: "hidden", lineHeight: 0 }}>
...
function PrimerReadout({ spec }) {
  const chip = { fontFamily: MN, fontSize: 11, color: P.dim, letterSpacing: "0.02em" };
  if (spec.kind === "spec") {
    return (
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", margin: "7px 0 0" }}>
        <span style={chip}>{spec.sef ? "SEF95 · band 0.5–45 Hz" : "time across · frequency up · colour is power"}</span>
      </div>
    );
  }
  return (
    <div style={{ display: "flex", gap: 14, flexWrap: "wrap", margin: "7px 0 0" }}>
      <span style={chip}>{spec.span} s window</span>
      <span style={chip}>{pGridLabel(spec.span)}</span>
    </div>
  );
}
...
function PrimerIndex({ opts, onOpen, onExit }) {
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
        <p style={{ flex: 1, minWidth: 0, fontFamily: MN, fontSize: 11, letterSpacing: "0.13em",
          textTransform: "uppercase", color: P.dim, margin: 0 }}>The basics</p>
        <ThemeButton />
      </div>
      <h1 style={{ fontFamily: UI, fontSize: 28, fontWeight: 700, letterSpacing: "-0.03em",
        lineHeight: 1.12, margin: "0 0 10px" }}>Before the modules</h1>
      <p style={{ fontFamily: UI, fontSize: 15.5, lineHeight: 1.5, color: P.ink, margin: "0 0 18px" }}>
        {PRIMER_CARDS.length} terms, read in order. Nothing here uses a word that has not already
        been explained. Every panel is live and is generated on this device.
      </p>

      {PRIMER_CARDS.map((c, i) => (
        <button key={c.id} onClick={() => onOpen(i)} style={{
          width: "100%", textAlign: "left", cursor: "pointer", border: `1px solid ${P.line}`,
          background: P.surface, borderRadius: 13, padding: "11px 13px", marginBottom: 9,
          minHeight: 74, display: "block",
        }}>
          <span style={{ display: "flex", alignItems: "center", gap: 13 }}>
            <span style={{ fontFamily: MN, fontSize: 11, color: P.dim, flex: "0 0 20px" }}>
              {i < 9 ? "0" + (i + 1) : String(i + 1)}
            </span>
            <span style={{ flex: "0 0 96px", height: 52 }}>
              <PrimerScope spec={pResolve(c, (c.ctrl && c.ctrl.def) || 0)} height={52} small />
            </span>
            <span style={{ flex: 1, minWidth: 0, display: "block" }}>
              <span style={{ display: "block", fontFamily: UI, fontSize: 16, fontWeight: 600,
                color: P.ink, lineHeight: 1.2 }}>{c.title}</span>
              <span style={{ display: "block", fontFamily: UI, fontSize: 12.5, lineHeight: 1.4,
                color: P.dim, marginTop: 3 }}>{c.tldr}</span>
            </span>
          </span>
        </button>
      ))}
      ...
      <div style={{ marginTop: 16 }}>
        <Btn kind="ghost" full onClick={onExit}>Done — go to the modules</Btn>
      </div>
    </div>
  );
}
...
function PrimerEntry({ idx, optIdx, setOpt, onGo, onBack, narrow }) {
  const c = PRIMER_CARDS[idx];
  const spec = pResolve(c, optIdx);
  const prev = PRIMER_CARDS[idx - 1], next = PRIMER_CARDS[idx + 1];
  const eyebrow = { fontFamily: MN, fontSize: 11, letterSpacing: "0.13em",
    textTransform: "uppercase", color: P.dim, margin: 0 };
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        <button onClick={onBack} aria-label="Back to all terms" style={{
          background: "none", border: "none", cursor: "pointer", minHeight: 44, padding: 0,
          fontFamily: UI, fontSize: 14, fontWeight: 500, color: P.accent,
        }}>&larr; All terms</button>
        <span style={{ ...eyebrow, marginLeft: "auto" }}>{idx + 1} of {PRIMER_CARDS.length}</span>
        <ThemeButton />
      </div>

      <PrimerScope spec={spec} height={narrow ? 152 : 180} />
      <PrimerReadout spec={spec} />

      {c.ctrl ? (
        <div>
          <p style={{ ...eyebrow, margin: "14px 0 0" }}>{c.ctrl.k}</p>
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            {c.ctrl.opts.map((o, i) => (
              <button key={String(o[1])} onClick={() => setOpt(i)} aria-pressed={i === optIdx} style={{
                flex: c.ctrl.opts.length === 4 ? "1 1 42%" : "1 1 28%", minWidth: 84, minHeight: 44, borderRadius: 10, cursor: "pointer",
                textAlign: "center", fontFamily: MN, fontSize: 12,
                background: i === optIdx ? P.ink : P.surface,
                color: i === optIdx ? P.surface : P.dim,
                border: `1px solid ${i === optIdx ? P.ink : P.line}`,
              }}>{o[1]}</button>
            ))}
          </div>
        </div>
      ...
      <p style={{ ...eyebrow, margin: "22px 0 0" }}>{c.topic}</p>
      <h1 style={{ fontFamily: UI, fontSize: narrow ? 27 : 30, fontWeight: 700,
        letterSpacing: "-0.03em", lineHeight: 1.12, margin: "6px 0 0" }}>{c.title}</h1>

      <div style={{ borderLeft: `3px solid ${P.accent}`, padding: "1px 0 1px 13px", margin: "13px 0 0" }}>
        <span style={{ ...eyebrow, color: P.accent, display: "block" }}>In one line</span>
        <p style={{ fontFamily: UI, fontSize: 16.5, lineHeight: 1.45, color: P.ink, margin: "4px 0 0" }}>{c.tldr}</p>
      </div>

      <p style={{ fontFamily: UI, fontSize: 17.5, lineHeight: 1.55, fontStyle: "italic",
        color: P.ink, margin: "22px 0 15px" }}>{c.lede}</p>

      {c.body.map((x, i) => (x.h
        ? <p key={i} style={{ fontFamily: UI, fontSize: 15, fontWeight: 700, lineHeight: 1.4,
            color: P.ink, margin: "24px 0 9px" }}>{x.h}</p>
        : <p key={i} style={{ fontFamily: UI, fontSize: 16, lineHeight: 1.6,
            color: P.ink, margin: "0 0 15px" }}>{x.p}</p>))}

      <div style={{ background: P.surface, border: `1px solid ${P.line}`, borderRadius: 11,
        padding: "13px 14px", margin: "22px 0 0" }}>
        <span style={{ ...eyebrow, display: "block", marginBottom: 7 }}>Sources</span>
        {c.src.map((s, i) => (
          <p key={i} style={{ fontFamily: UI, fontSize: 13.5, lineHeight: 1.55, color: P.ink,
            margin: i === c.src.length - 1 ? 0 : "0 0 7px" }}>{withDoi(s.replace(/<\/?em>/g, ""), i)}</p>
        ))}
      </div>

      <div style={{ display: "flex", gap: 10, margin: "26px 0 0" }}>
        <button onClick={() => onGo(prev ? idx - 1 : -1)} style={{
          flex: 1, minHeight: 60, display: "block", padding: "10px 14px", cursor: "pointer",
          border: `1px solid ${P.line}`, borderRadius: 12, background: P.surface, textAlign: "left",
        }}>
          <span style={{ display: "block", ...eyebrow }}>&larr; {prev ? "Previous" : "Back"}</span>
          <span style={{ display: "block", fontFamily: UI, fontSize: 15, fontWeight: 600,
            color: P.ink, marginTop: 3 }}>{prev ? prev.title : "All terms"}</span>
        </button>
        <button onClick={() => onGo(next ? idx + 1 : -1)} style={{
          flex: 1, minHeight: 60, display: "block", padding: "10px 14px", cursor: "pointer",
          border: `1px solid ${P.line}`, borderRadius: 12, background: P.surface, textAlign: "right",
        }}>
          <span style={{ display: "block", ...eyebrow }}>{next ? "Next" : "Done"} &rarr;</span>
          <span style={{ display: "block", fontFamily: UI, fontSize: 15, fontWeight: 600,
            color: P.ink, marginTop: 3 }}>{next ? next.title : "All terms"}</span>
        </button>
      </div>
    </div>
  );
...
/* ---- the primer view ------------------------------------------------------
   Sits where Session sits: a top-level branch that bypasses the tab shell,
   because the primer is an on-ramp rather than a destination.                 */

function Primer({ prog, persist, onExit, narrow }) {
  const start = (prog.primer && prog.primer.at) || 0;
  const [idx, setIdx] = useState(start > 0 ? start : -1);   // -1 = the index screen
  const [opts, setOpts] = useState({});

  useEffect(() => {
    persist({ ...prog, primer: { seen: true, at: idx < 0 ? 0 : idx } });
  }, [idx]); // eslint-disable-line

  const card = idx >= 0 ? PRIMER_CARDS[idx] : null;
  const optIdx = card ? (opts[card.id] !== undefined ? opts[card.id] : (card.ctrl && card.ctrl.def) || 0) : 0;
  const announce = card
    ? card.title + ", term " + (idx + 1) + " of " + PRIMER_CARDS.length + ". " + card.tldr
    : "The basics, all " + PRIMER_CARDS.length + " terms";

  return (
    <div style={{ minHeight: "100vh", background: P.bg, color: P.ink, fontFamily: UI }}>
      <Styles />
      <p aria-live="polite" aria-atomic="true" style={{
        position: "absolute", width: 1, height: 1, margin: -1, padding: 0,
        overflow: "hidden", whiteSpace: "nowrap", border: 0, clip: "rect(0 0 0 0)",
      }}>{announce}</p>
      <main style={{ maxWidth: 620, margin: "0 auto", padding: "20px 17px 44px" }}>
        {idx < 0
          ? <PrimerIndex opts={opts} onOpen={setIdx} onExit={onExit} />
          : <PrimerEntry idx={idx} optIdx={optIdx} narrow={narrow}
              setOpt={(i) => { if (card) setOpts({ ...opts, [card.id]: i }); }}
              onGo={setIdx} onBack={() => setIdx(-1)} />}
      </main>
    </div>
  );
}
...
export function App() {
  ...
  const [inPrimer, setInPrimer] = useState(false);
  ...
  const [theme, setTheme] = useState(P.name);
  const { narrow } = useViewport();

  useEffect(() => {
    loadProgress().then((p) => {
      if (p) setProg(p);
      setTheme(applyTheme((p && p.theme) || "dark"));
  ...
  // applyTheme mutates P in place, so the re-render triggered by setTheme is
  // what repaints the tree; Scope picks the new colours up on its next frame
  // without unmounting, which is why the spectrogram history survives.
  const chooseTheme = (name) => {
    const t = applyTheme(name);
    setTheme(t);
    persist({ ...prog, theme: t });
  };
  setThemeFromAnywhere = chooseTheme;
  ...
  // This used to only write seenIntro: false. showIntro is state, set once at
  // mount, so writing the flag deferred the whole effect to the next launch and
  // the control looked dead. It also has to leave More, because the landing
  // only renders on Learn / home.
  const showOpening = () => {
    persist({ ...prog, seenIntro: false });
    setShowIntro(true);
    setTab("learn");
    setView("home");
  };
  ...
  if (inPrimer) {
    return <Primer prog={prog} persist={persist} narrow={narrow} onExit={() => setInPrimer(false)} />;
  }
  ...
  // Tapping a tab swaps the whole screen with no focus change, so a screen
  // reader user was left where they stood with nothing announced. This string
  // only changes when the screen does, so the live region does not chatter.
  const announce =
    tab !== "learn" ? { drill: "Drill", bench: "Bench", ref: "Reference", more: "More" }[tab]
    : view === "module" ? `${MODULES[mi].n}, ${MODULES[mi].title}, point ${pi + 1} of ${MODULES[mi].points.length}`
    : view === "check" ? `Module check, ${MODULES[mi].title}`
    : showIntro ? "Reading the EEG, opening screen"
    : "All modules";
      ...
      <main style={{ maxWidth: 620, margin: "0 auto", padding: "20px 17px 108px" }}>
        {tab === "learn" && view === "home" && showIntro && (
          <Landing
            ...
            onPrimer={() => { persist({ ...prog, seenIntro: true }); setShowIntro(false); setInPrimer(true); }}
          />
        )}
        {tab === "learn" && view === "home" && !showIntro && (
        ...
        {tab === "ref" && <RefScreen />}
        {tab === "more" && <MoreScreen prog={prog} persist={persist} theme={theme} onTheme={chooseTheme} onShowIntro={showOpening} />}
      </main>

      <nav style={{
        position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 10,
        background: tint(P.bg, 0.94), borderTop: `1px solid ${P.line}`,
  ...
  render() {
    ...
    return React.createElement("div",
      ...
      React.createElement("div", { style: { maxWidth: 560, margin: "0 auto" } },
        ...
        React.createElement("p", { style: { fontSize: 15.5, lineHeight: 1.55, color: P.ink, margin: "0 0 16px" } },
...
function Styles() {
  return (
    <style>{`
      /* No web font is fetched. The page states on the landing screen and in
         More > About that nothing is downloaded and no data leaves the device,
         and a Google Fonts import would have made all three of those sentences
         false while sending every reader's IP and referrer to Google. The font
         stacks below already name system faces as fallbacks, so the type
         degrades to SF / Georgia / SF Mono rather than breaking. */
